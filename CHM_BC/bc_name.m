...
%
%     <prefix>_<N>_<defect>_<#Lambda>.dat        e.g.  VH_10_0_109.dat
%
...
%
%     N <=  40   <prefix>_<N>_<d>_<L>.dat
%     N <=  60   <prefix>_<N>_<d>.dat            (#Lambda too slow)
%     N >   60   <prefix>_<N>.dat                (both too slow)
%
% A letter is appended when the name is taken, so a second solution with the
% same invariants does not overwrite the first: VH_14_0_687A.dat, ...B, ...
%
...
% ------------------------------------------------------------------------------
    ...
    while exist(name, 'file')
        ...
        name = sprintf('%s%s.dat', stem, letters(k));
    end
